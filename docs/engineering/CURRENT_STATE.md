...
- Fixture tests now resolve `data/canonical_specs` from the test fileâ€™s backend root, independent of process cwd; fixture contents and production code are unchanged.
...
- Current 7-day and 30-day growth is zero for timestamped audited labels. The May 22â€“27 active cohort grew at approximately 17,600 timestamped nodes/day, so cleanup buys time but migration remains required.
...
- Seven local production-build tests passed at 1440Ã—900 and 390Ã—844. API requests were mocked; no production writes or credentials were used.
- Browser-confirmed fixes: Escape/outside-click mobile drawer closure and a usable branded 404 home link.
- Remaining browser work is fixture-backed manual/generated workflow states, shared-build runtime coverage, 1280Ã—800/768Ã—1024 viewports, and automated axe scoring.
...
- Added 30-product deterministic pagination coverage for the current â€œLoad more productsâ€ contract at desktop, tablet, and mobile sizes.
...
- Implemented `buildcores_import_cli.py` (new Cloud SQLâ€“aware CLI with dry-run, import, idempotency, and verify commands).
- Dry-run: 25,699 files discovered, 280 records selected, 0 duplicates, 0 price/offer/image data â€” PASSED.
...
- Verification: 0 store offers, 0 price history, 0 images, 0 duplicate GTINs, 0 duplicate brand+MPN â€” PASSED.
- Idempotency: 280 scanned, 0 would insert â€” PASSED.
...
- All 280 BuildCores products bulk-approved (pendingâ†’approved) before deployment so catalog API returns full 280-product set.
...
- Prerequisite CI: `29697610593` â€” PASSED.
